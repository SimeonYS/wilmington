...
class WilmingtonPipeline:

    # Database setup
    conn = sqlite3.connect('wilmington.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `wilmington`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM wilmington WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO wilmington (date, title, link, content)"
